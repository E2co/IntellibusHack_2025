    ...
    <div className="min-h-screen bg-gradient-to-br from-background via-primary/5 to-secondary/10 pb-8">
      ...
      <div className="max-w-7xl mx-auto space-y-8 pt-20 p-4 md:p-8">
        ...
        <div className="text-center space-y-2 md:space-y-4 animate-slide-up">
          <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold">Live Traffic at TAJ</h1>
          <p className="text-base md:text-lg text-muted-foreground">facilitated by QueMeNow</p>
          <p className="text-xs md:text-sm text-muted-foreground">Updated just now</p>
        </div>
        ...
        <div className="hidden md:block relative min-h-[800px]">
          ...
          {tiles.map((service, index) => {
            ...
            return (
              ...
              >
                <div className="w-48">
                  <ServiceTile {...service} onClick={() => navigate("/signup")} />
                </div>
              </div>
            );
          })}
        ...
        <div className="md:hidden space-y-6">
          ...
          <GlassCard className="p-6 flex flex-col items-center justify-center space-y-4 animate-slide-up mx-auto max-w-sm">
            <StatusChip status={status} />
            <div className="text-center space-y-2">
              <div className="text-3xl font-bold">{status === 'busy' ? 'Busy' : status === 'moderate' ? 'Moderate' : 'Light'}</div>
        ...
        <GlassCard className="p-3 md:p-4 text-center text-xs md:text-sm text-muted-foreground">
          <div className="animate-pulse-glow">{error ? `⚠️ ${error}` : "🔴 Live • System operational • All services available"}</div>
        </GlassCard>
      </div>
    </div>
  );
