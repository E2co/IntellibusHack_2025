...
export default function Dashboard() {
    ...
    const filteredQueue =
        serviceFilter === 'all'
            ? waitingQueue
            : waitingQueue.filter((entry) => String(entry.serviceId) === serviceFilter);
        ...
        <div className="space-y-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {activeSessions.length === 0 ? (
                        ...
                        activeSessions.map((session) => {
                            ...
                            return queueEntry ? (
                                <ActiveSessionCard key={session.id} session={session} queueEntry={queueEntry} />
                            ) : null;
                        })
                    )}
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-6">
                {services.map((service) => (
                    <ServiceStatBox
                        key={service.id}
                        count={stats.serviceStats[String(service.id)]?.waiting || 0}
                        label={service.name}
                        icon={service.icon}
                    />
                ))}
